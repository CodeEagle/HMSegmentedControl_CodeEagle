...
extension Array {
	subscript(hm_safe index: Int) -> Element? {
	...
	fileprivate func drawText(_ oldRect: CGRect) {
		...
		for idx in 0 ..< titles.count {
			...
			if segmentWidthStyle == .fixed {
				...
				let widthForIndex: CGFloat = (segmentWidthsArray[hm_safe: idx] ?? 0)
	...
	fileprivate func drawImage() {
		...
		for (idx, iconImage) in value.enumerated() {
			...
			if selectedSegmentIndex == idx {
				imageLayer.contents = sectionSelectedImages?[hm_safe: idx]?.cgImage ?? icon.cgImage
			}
	...
	fileprivate func drawTextImage() {
		...
		for (idx, iconImage) in value.enumerated() {
			...
			if segmentWidthStyle == .fixed {
				...
				let width = segmentWidthsArray[hm_safe: idx] ?? 0
